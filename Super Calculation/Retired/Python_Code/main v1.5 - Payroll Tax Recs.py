...
paycode_summary.to_csv(os.path.join(OUTPUT_DIR, 'Paycode_Summary_new_input_file_fullFYs.csv'), index=False)
...
# Export to CSV
unique_comments.to_csv(os.path.join(OUTPUT_DIR, 'unique_comments.csv'), index=False)
...
pd.DataFrame(Discp1_unique_QtrEMPLID, columns=['QtrEMPLID']).to_csv(os.path.join(OUTPUT_DIR, 'Discp1_unique_QtrEMPLID.csv'), index=False)
