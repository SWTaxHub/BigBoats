...
Payroll_Offshore_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\MARITIMO OFFSHORE\Payroll"


#Declare File path for Allowance Paycodes 
allowancePaycodes_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\Allowances_crossEntity.csv"
#Declare File path for Contribution Paycodes 
contributionPaycodes_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\Contributions_crossEntity.csv"
#Declare File path for Deductions Paycodes 
deductionsPaycodes_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\Deductions_crossEntity.csv"
#Declare File path for Income Paycodes 
incomePaycodes_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\Income_crossEntity.csv"
#Declare File path for Employee Labels Labour
Employee_labelsLabour_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\MARITIMO LABOUR\Employee details\Employee_Labels.csv"
#Declare File path for Employee Labels Offshore
Employee_labelsOffshore_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\MARITIMO OFFSHORE\Employee details\Employee_Labels.csv"
# Declare File path for Super Labour
Super_Labour_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\MARITIMO LABOUR\Super\CSVs"
# Declare file path for super offshore
Super_Offshore_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\MARITIMO OFFSHORE\Super\CSVs"
# Declare file path for combo paycodes
Combo_Paycodes_filepath = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\Shared Folder\Payroll reports\Paycode_CrossEntity.csv"
...
def payroll_calc(Payroll_Offshore_data, combo_Paycodes, file_suffix="LABOUR / OFFSHORE"):
   ...
   #print(merged_data.isnull().sum())  # Check for null values
    ...
    PayMap = r"C:\Users\USER\OneDrive - SW Accountants\Desktop\Maritimo\2025.05.21_PAYCODE_MAPPING.xlsx"
    ...
    merged_data['Client Map - OTE (not capped)'] = np.where(
        merged_data['Combined_PayCode'].isin(OTE_paycodesBigBoats),
        merged_data['Total'],
        0
    )
    
    #merged_data['SW Mapping - OTE'] 
    merged_data['SW Map - OTE (not capped)']= np.where(
        merged_data['Combined_PayCode'].isin(OTE_paycodesSW),
        merged_data['Total'],
        0
    )

    merged_data['SW Map - S&W (not capped)'] = np.where(
        merged_data['Combined_PayCode'].isin(SnW_paycodesSW),
        merged_data['Total'],
        0
    )
    ...
    merged_data['Client Map - OTE SG (Not capped)'] = np.where(
        merged_data['Combined_PayCode'].isin(OTE_paycodesBigBoats),
        merged_data['Total'] * merged_data['SG_Rate'],
        0
    )

    merged_data['Client Map - OTE SG (Not capped)'] = merged_data['Client Map - OTE SG (Not capped)'].astype(float).round(2)

    
    
    merged_data['SW Map - OTE SG (Not capped)'] = np.where(
        merged_data['Combined_PayCode'].isin(OTE_paycodesSW),
        merged_data['Total'] * merged_data['SG_Rate'],
        0
    )

    merged_data['SW Map - OTE SG (Not capped)'] = merged_data['SW Map - OTE SG (Not capped)'].astype(float).round(2)


    merged_data['SW Map - S&W SG (Not capped)'] = merged_data['SW Map - S&W (not capped)'] * merged_data['SG_Rate']
    ...
    #merged_data['S&W - Client to SW Map Discrepancy'] = merged_data['Client Mapping - S&W'] - merged_data['SW Map - S&W (not capped)']

    merged_data['OTE SG Expected - Client to SW Map Discrepancy'] = merged_data['Client Map - OTE SG (Not capped)'] - merged_data['SW Map - OTE SG (Not capped)']

    #merged_data['S&W SG Expected - Client to SW Map Discrepancy'] = merged_data['Client Map - S&W SG'] - merged_data['SW Map - S&W SG (Not capped)']

    # # Compute the difference
    # merged_data['Super_Diff'] = (
    #     merged_data['SW Map - OTE SG (Not capped)'] - merged_data['Client Map - OTE SG (Not capped)']
    # )
...
# Commented out as this function is not used in the current code flow - 5/06/2025
...
#         (relevant_rows['Client Map - OTE SG (Not capped)'] -
#          relevant_rows['SW Map - OTE SG (Not capped)']) != 0
#     ]
...
#          relevant_rows['Client Map - OTE SG (Not capped)']) != 0
#     ]
