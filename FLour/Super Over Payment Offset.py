...
def calculate_shortfall_offset(Payroll_data):
    agg_methods = {
        ...
        #'Pin' : 'first',
        'Qtr_Key' : 'first',
        #'Pay_Attribute_Code' : 'first',
       'Pay_Period_Quarter' : 'first',
        #'Pay_Attribute_Description': 'first',
        #'Additional_Super_Rate' : 'first',
        'Hrs': 'sum',
        'Amt': 'sum',
        'SG_Rate': 'mean',
        'Pay_Period_Quarter': 'first',
...
# Commented out as PIN is not equivalent to Pay_Number
   # df1 = mergedData_Labour.groupby(['Period_Ending_Date', 'Pin', 'Emp_Code']).agg(agg_methods).reset_index()
    ...
    df1['cumulative_sum_12Months_OVERPAY'] = df1.groupby('Emp_Code').apply( # Groupd the DataFrame by 'Emp_Code' so we process each employee separately
        lambda g: g.apply(
            # apply() will operate on each group (i.e. all rows for one employee).
            # For each group g (i.e. one employee’s data), we're applying another function row by row.
            lambda row: g.loc[
                # For each row, we locate all other rows within the same group (employee) that meet the following conditions:
